...
The simulator can load and executing binary machine code prepared with
...
and indices are invariably constants, this gives maximum opportunity
...
@ We introduce an enumerated type for ALU functions, so we don't
...
that writes some of the flag bits writes all of them, and shifts write
the $C$ flag even if the shift amount is zero.
...
rule applied to the {\tt mvn} instruction, so we follow that
convention here.
...
|cflag| to the last bit shifted out, or~|0| if~|n=0|.  Left and right
shifts are easy, though we have to be careful to make sure \CEE/ gives
us an arithmetic shift for |Asr| by inserting appropriate casts.  The
|Ror| operation has to be decomposed into two shifts: we hope the
\CEE/ compiler has the smarts to combine them into an {\tt ror}
...
static Word shifter(ShiftOp op, Word x, int n, Bool *cflag)@+{
    ...
    switch (op) {
    case Lsl:
        r = x << n; if (n > 0) c = bit(x, 32-n);@+break; 
    case Lsr:
        r = x >> n; if (n > 0) c = bit(x, n-1);@+break;
    case Asr:
        r = (Word) ((int32_t) x >> n);
        if (n > 0) c = bit(x, n-1);
        break;
...
five-bit immediate field, and still others take the shift amount from
the low-order bits of the first register |ra| read by the
instruction.  We will introduce an enumerated type to list the
possibilities.

@<Enumerations...@>=
typedef enum {
    @!Sh0, @!Sh1, @!Sh2, @!Sh12, @!ShImm, @!ShReg
} ShiftSel;

@ A function |shiftsel| interprets a value of this type by selecting
the appropriate source.  If the shift amount comes from a register,
then the shift amount is reduced modulo~32.
...
@<Enumerations for signals@>=
typedef enum { @!yes, @!no, @!condx } Perhaps;
...
to~|condx|.  It corresponds to a three-way multiplexer in the hardware.
...
values |yes|, |no| and |condx|, meaning that the answer will be
...
satisfied, and a value of |condx| here means that the write is conditional.
...
opcodes: these could become ROMs in a hardware implementation.  The
majority of instructions |instr| can be decoded by looking up the five
bits |field(instr, 16, 11)| in table |decode1|, unless those bits are
$01000$, in which case we must consult one of two other tables.  We
don't implement byte and halfword loads and stores, nor {\tt push},
...
o("lsrs", __, Ry, Rx, RegB, Lsr, ShImm, Mov, F, F, T, Y, N), /* 1 */
o("asrs", __, Ry, Rx, RegB, Asr, ShImm, Mov, F, F, T, Y, N), /* 2 */
...
It's good to distinguish between the {\it decoded\/} signals -- those
that appear in the decoding tables and are determined by the
instruction's opcode -- the {\it derived\/} signals that also depend on
other parts of the instruction halfword, and {\it dynamic\/} signals
that depend on other parts of the machine state.
\begingroup\multiply\parindent by2 \parskip=\medskipamount
\bullitem The decoded signals
...
\bullitem The derived signals will be the same whenever a particular
...
\bullitem The dynamic signals differ from one execution of the
...
put together a simple main program to drive it.  First we gather
together in proper order all the code we have accumulated so far.
